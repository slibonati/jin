 ...
 * server.
 */
public class JinFreechessConnection extends FreechessConnection implements Connection, SeekConnection, PGNConnection {
	...
	public JinFreechessConnection(String requestedUsername, String password) {
		...
		setInterface(Jin.getAppName() + " " + Jin.getAppVersion() + " (" + System.getProperty("java.vendor") + " "
				+ System.getProperty("java.version") + ", " + System.getProperty("os.name") + " " + getSafeOSVersion()
				+ ")");
	...
	/**
	 * Returns the OS version after stripping out the patch level from it. We do
	 * this to avoid revealing that information to everyone on the server.
	 */
	...
	/**
	 * Returns a Player object corresponding to the specified string. If the string
	 * is "W", returns <code>Player.WHITE</code>. If it's "B", returns
	...
	/**
	 * If we're logged in, sends the specified command to the server, otherwise the
	 * command is put into a queue and sent on-login.
	 */
	...
	/**
	 * Overrides {@link free.util.Connection#connectImpl(String, int)} to return a
	 * timesealing socket.
	 */
	@Override
	 ...
	 */
	private static final Pattern TOURNEY_TELL_REGEX = Pattern
			.compile("^(" + USERNAME_REGEX + ")(" + TITLES_REGEX + ")?\\(T(\\d+)\\): (.*)");
	...
	protected boolean processQTell(String message) {
		...
		if (matcher.matches()) {
			...
			evt = new ChatEvent(this, null, "qtell.tourney", ChatEvent.TOURNEY_CHAT_CATEGORY, userForName(sender),
					title, -1, message, tourneyIndex);
		} else {
			evt = new ChatEvent(this, null, "qtell", ChatEvent.PERSON_TO_PERSON_CHAT_CATEGORY, null, null, -1, message,
					null);
		}
	...
	private static WildVariant getVariant(String categoryName) {
		if (categoryName.equalsIgnoreCase("lightning") || categoryName.equalsIgnoreCase("blitz")
				|| categoryName.equalsIgnoreCase("standard") || categoryName.equalsIgnoreCase("untimed"))
		...
		// This means it's a fake variant we're using because the server hasn't told us
		// the real one.
	 ...
	 * that can be used for issuing a seek, e.g. "w1" or "fr". Returns null if the
	 * specified wild variant is not supported by FICS.
	 */
	...
	public WildVariant[] getSupportedVariants() {
		if (wildVariants == null) {
			wildVariants = new WildVariant[] { Chess.getInstance(), FischerRandom.getInstance(), Suicide.getInstance(),
					Atomic.getInstance(), new ChesslikeGenericVariant(Chess.INITIAL_POSITION_FEN, "losers"),
	...
	/**
	 * A hashtable where we keep game numbers mapped to GameInfoStruct objects of
	 ...
	 * these games but the server didn't tell us yet whether the move is legal or
	 * not.
	 */
	...
	/**
	 * The user's primary played (by the user) game, -1 if unknown. This is only set
	 * when the user is playing more than one game.
	 */
	private int primaryPlayedGame = -1;

	/**
	 * The user's primary observed game, -1 if unknown. This is only set when the
	 * user is observing more than one game.
	 */
	private int primaryObservedGame = -1;

	/**
	 * Returns the game with the specified number. This method (currently) exists
	 * solely for the benefit of the arrow/circle script.
	 */
	...
	/**
	 * Returns the InternalGameData for the ongoing game with the specified number.
	 * Throws a <code>NoSuchGameException</code> if there's no such game.
	 */
	...
	/**
	 * Finds the played user's game against the specified opponent. Returns the game
	 * number of null if no such game exists.
	 */
	...
	protected boolean processStyle12(Style12Struct boardData) {
		...
		if (unstartedGameInfo != null) // A new game
			...
			GameInfoStruct fakeGameInfo = new GameInfoStruct(boardData.getGameNumber(), false, "Unknown variant", false,
					false, false, boardData.getInitialTime(), boardData.getIncrement(), boardData.getInitialTime(),
					boardData.getIncrement(), 0, -1, ' ', -1, ' ', false, false);
	...
	protected boolean processDeltaBoard(DeltaBoardStruct data) {
		...
		boolean canWhiteCastleKingside = lastBoardData.canWhiteCastleKingside() && !kingMoved
				&& !Square.getInstance(7, 0).equals(startSquare);
		boolean canWhiteCastleQueenside = lastBoardData.canBlackCastleQueenside() && !kingMoved
				&& !Square.getInstance(0, 0).equals(startSquare);
		boolean canBlackCastleKingside = lastBoardData.canBlackCastleKingside() && !kingMoved
				&& !Square.getInstance(7, 7).equals(startSquare);
		boolean canBlackCastleQueenside = lastBoardData.canBlackCastleQueenside() && !kingMoved
		...
		Style12Struct boardData = new Style12Struct(boardLexigraphic, currentPlayer, doublePawnPushFile,
				...
				pliesSinceIrreversible, gameNumber.intValue(), whiteName, blackName, gameType, isPlayedGame, isMyTurn,
				initTime, inc, whiteStrength, blackStrength, whiteTime, blackTime, nextMoveNumber, moveVerbose, moveSAN,
				moveTime, isBoardFlipped, isClockRunning, lag);
	...
	private static class InternalGameData {
		 ...
		 * <code>Player</code> who made the offer and the 2nd is the offer id. Takeback
		 ...
		 * where the first element is the <code>Player</code> who made the offer and the
		 * 2nd is an <code>Integer</code> specifying the amount of plies offered to take
		 * back.
		 */
		public final Hashtable indicesToTakebackOffers = new Hashtable();

		/**
		 * Works as a set of the offers currently in this game. The elements are Pairs
		 * in which the first item is the <code>Player</code> who made the offer and the
		...
		/**
		 * Returns true if the specified offer is currently made by the specified player
		 * in this game.
		 */
	...
	/**
	 * This method informs the user that he tried to use (observe, play etc.) a wild
	 * variant not supported by Jin. Please use this method when appropriate instead
	 * of sending your own message.
	 */
	protected void warnVariantUnsupported(String variantName) {
		Object[] messageFormatArgs = new Object[] { Jin.getAppName(), variantName };
		String message = I18n.get(JinFreechessConnection.class).getFormattedString("unsupportedVariantMessage",
				messageFormatArgs);
	 ...
	 */
	private static TimeControl getTimeControl(int whiteInitial, int whiteIncrement, int blackInitial,
			int blackIncrement) {
	...
	/**
	 * Called when a new game is starting. Responsible for creating the game on the
	 * client side and firing appropriate events. Returns an InternalGameData
	...
	private InternalGameData startGame(GameInfoStruct gameInfo, Style12Struct boardData) {
		...
		TimeControl timeControl = getTimeControl(gameInfo.getWhiteTime(), gameInfo.getWhiteInc(),
				gameInfo.getBlackTime(), gameInfo.getBlackInc());
		...
		Game game = new Game(gameType, initPos, boardData.getPlayedPlyCount(), whiteName, blackName, timeControl,
				whiteRating, blackRating, gameID, categoryName, isRated, isPlayed, whiteTitles, blackTitles,
				initiallyFlipped, userPlayer);
	...
	private void updateGame(InternalGameData gameData, Style12Struct boardData) {
		...
		game.setTimeControl(getTimeControl(boardData.getInitialTime(), boardData.getIncrement(),
				boardData.getInitialTime(), boardData.getIncrement()));
	...
	/**
	 * Returns whether <code>echoedMove</code> (sent to us by the server) is the
	 * same move as <code>sentMove</code> (a move we sent to the server).
	 */
	...
	/**
	 * Determines whether it's possible to issue a takeback for the specified game
	 * change and if so calls issueTakeback, otherwise calls changePosition.
	 */
	...
	private void changePosition(InternalGameData gameData, Style12Struct newBoardData) {
		...
		// We do this because moves in bsetup mode cause position change events, not
		// move events
	...
	protected boolean processSeekAdded(SeekInfoStruct seekInfo) {
		...
		if (getRequestedIvarState(Ivar.SEEKINFO)) {
			...
			if (variant != null) {
				...
				boolean isRatingLimited = ((seekInfo.getOpponentMinRating() > 0)
						|| (seekInfo.getOpponentMaxRating() < 9999));

				TimeControl timeControl = new FischerTimeControl(seekInfo.getMatchTime() * 60 * 1000,
						seekInfo.getMatchIncrement() * 1000);

				Seek seek = new Seek(seekID, userForName(seekInfo.getSeekerHandle()), titlesBuf.toString(),
						seekInfo.getSeekerRating(), isProvisional, isRegistered, isSeekerRated, isComputer, variant,
						seekInfo.getMatchType(), timeControl, seekInfo.isMatchRated(), color, isRatingLimited,
						seekInfo.getOpponentMinRating(), seekInfo.getOpponentMaxRating(), !seekInfo.isAutomaticAccept(),
	 ...
	 * generated by this SeekJinConnection and it must be in the current sought
	 * list.
	 */
	@Override
	...
	public void issue(UserSeek seek) {
		...
		String seekCommand = "seek " + seek.getTime() + " " + seek.getInc() + " "
				...
				+ (color == null ? "" : color.isWhite() ? "white " : "black ") + wildName + " "
				+ (seek.isManualAccept() ? "manual " : "") + (seek.isFormula() ? "formula " : "")
	...
	/**
	 * Maps offer indices to the <code>InternalGameData</code> objects representing
	 * the games in which the offer was made.
	 */
	private final Hashtable offerIndicesToGameData = new Hashtable();

	/**
	 * Override processOffer to always return true, since we don't want the user to
	 * ever see these messages.
	 */
	@Override
	protected boolean processOffer(boolean toUser, String offerType, int offerIndex, String oppName,
			String offerParams) {
	 ...
	 */
	@Override
	protected boolean processMatchOffered(boolean toUser, int offerIndex, String oppName, String matchDetails) {
	 ...
	 */
	@Override
	protected boolean processTakebackOffered(boolean toUser, int offerIndex, String oppName, int takebackCount) {
		...
		try {
			...
			gameData.indicesToTakebackOffers.put(new Integer(offerIndex), new Pair(player, new Integer(takebackCount)));
	...
	/**
	 * Gets called by the various process[offerType]Offered() methods to handle the
	 * offers uniformly.
	 */
	...
	protected boolean processOfferRemoved(int offerIndex) {
		...
		InternalGameData gameData = (InternalGameData) offerIndicesToGameData.remove(new Integer(offerIndex));
	 ...
	 */
	@Override
	protected boolean processPlayerCounteredTakebackOffer(int gameNum, String playerName, int takebackCount) {
	...
	/**
	 * Returns the offerId (as defined by OfferEvent) corresponding to the specified
	 * offer name. Throws an IllegalArgumentException if the offer name is not
	 * recognizes.
	 */
	...
	private void updateOffers(InternalGameData gameData, int offerId, Player player, boolean on) {
		...
		if (offerId == OfferEvent.TAKEBACK_OFFER) {
			// We're forced to fake this so that an event is fired even if we start
			// observing a game
			...
			// We check this because we might get such an event if we start observing a game
			// with
			// an existing offer.
	 ...
	 * <code>IllegalArgumentException</code> if the move is not of a type that we
	 * know how to send to the server.
	 */
	...
	/**
	 * Sends a request to draw the given game. The given game must be a played game
	 * and of type Game.MY_GAME.
	 */
	@Override
	...
	/**
	 * Sends a request to abort the given game. The given game must be a played game
	 * and of type Game.MY_GAME.
	 */
	@Override
	...
	/**
	 * Sends a request to rematch after a game ends. The given game must be a played
	 * game and of type Game.MY_GAME.
	 */
	...
	public void requestRematch(Game game) {
		...
		sendCommand("rematch", true, true, false);
	}
	...
	/**
	 * Goes back the given amount of plies in the given game. If the given amount of
	 * plies is bigger than the amount of plies since the beginning of the game,
	...
	/**
	 * Overrides Connection.execRunnable(Runnable) to execute the runnable on the
	 * AWT thread using SwingUtilities.invokeLater(Runnable), since this class is
	 * meant to be used by Jin, a graphical interface using Swing.
	 *
