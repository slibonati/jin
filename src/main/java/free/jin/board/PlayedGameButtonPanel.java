...
public class PlayedGameButtonPanel extends FixedJPanel implements ActionListener {
	 ...
	 */
	private static final Border OFFERED_STATE_BORDER = new EmptyBorder(STATE_BORDER_SIZE, STATE_BORDER_SIZE,
			STATE_BORDER_SIZE, STATE_BORDER_SIZE);
	 ...
	 */
	private static final Border OFFER_STATE_BORDER = new EmptyBorder(STATE_BORDER_SIZE, STATE_BORDER_SIZE,
			STATE_BORDER_SIZE, STATE_BORDER_SIZE);
	...
	protected JButton rematchButton;

	/**
	 * The button for the border of the rematch button.
	 */
	...
	/**
	 * The game listener that gets notified of various game events we're interested
	 * in.
	 */
	private GameListener gameListener = new GameAdapter() {
		...
		public void gameEnded(GameEndEvent evt) {
			...
			if (rematchButton != null && game.getResultCode() != Game.UNKNOWN_RESULT)
	...
	/**
	 * Creates a new PlayedGameButtonPanel. It will be used by the given Plugin for
	 * the given Game. The given parent Component determines over which component
	...
	/**
	 * A list of the ply counts for all of the user's current outstanding takeback
	 * offers.
	 */
	...
	protected void createComponents(Plugin plugin, Game game) {
		...
		takebackNButton = conn.isMultipleTakebackSupported() ? createButton("multipleTakebackButton") : null;
		rematchButton = conn.isRematchSupported() ? createButton("rematchButton") : null;
	}
	...
	protected void addComponents(Plugin plugin, Game game) {
		...
		if (rematchButton != null) {
			rematchButtonPanel = new JPanel(WrapLayout.getInstance());
			rematchButtonPanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, Integer.MAX_VALUE));
			rematchButtonPanel.setBorder(OFFERED_STATE_BORDER);
			rematchButtonPanel.add(rematchButton, BorderLayout.CENTER);
			add(rematchButtonPanel);
		}
	...
	/**
	 * ActionListener implementation. Executes the appropriate command depending on
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (source == resignButton) {
			Object result = I18n.get(PlayedGameButtonPanel.class).confirm(OptionPanel.OK, "resignConfirmation",
					parentComponent);
		...
		else if (source == rematchButton) {
			conn.requestRematch(game);
			rematchButton.setEnabled(false);
