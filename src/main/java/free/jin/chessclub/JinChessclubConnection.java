...
public class JinChessclubConnection extends ChessclubConnection
    implements DatagramListener, Connection, SeekConnection, GameListConnection, PGNConnection,
        FriendsConnection, MatchOfferConnection {
  ...
  /**
   * Called when a datagram we're interested in arrives from the server (implementation of
   * <code>DatagramListener</code>). This method simply does a huge switch on the id of the datagram
   * and dispatches the actual handling to the appropriate method.
   */
  @Override
  ...
  /**
   * <P>
   * Returns the GameInfo object for the given game number. This method throws a
   * <code>NoSuchGameException</code> if there is no started game with the given number. It is
   * important to throw it here and to force the user to catch the exception and handle it. This can
   * always happen if the server thinks the game started, but the client doesn't (if the wild
   * variant is not supported for example). What will happen in this case is that the datagrams will
   * continue to arrive, but all the datagram processing code must ignore them. Forcing the datagram
   ...
   * <P>
   * Of course you can always ignore this method and access <code>gameNumbersToGameInfo</code>
   * directly.
   */
